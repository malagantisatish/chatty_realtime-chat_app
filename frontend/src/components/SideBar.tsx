...
const SideBar = () => {
  ...
  const filteredUsers = showOnlineOnly ? users.filter((user) => onlineUsers.includes(user._id) && user._id) : users
  if (isUsersLoading) {
            ...
            >
              <div className="relative mx-auto lg:mx-0">
                ...
                {onlineUsers.includes(user._id) && (
                  <span
                    className="border-[0.5px] absolute bottom-0 right-0 size-3 bg-green-500 
                  rounded-full ring-2"
                  />
                )}
              </div>
